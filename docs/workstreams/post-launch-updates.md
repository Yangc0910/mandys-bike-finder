...
- Status: Local, protected-preview, simulator launch/recovery, and native screenshot source capture passes complete on 2026-06-11; TestFlight verification awaits a Capacitor-accessible staging URL.
...
Provide a stable release-candidate staging URL that does not require interactive Vercel authentication inside Capacitor. Then verify destructive-action confirmations and TestFlight, complete the marketing-frame review from the captured `1320 x 2868` native sources, and upload the approved set to App Store Connect.
